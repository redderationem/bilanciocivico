...
class Authenticate{
	...
	public function __invoke(Request $request, Response $response, callable $next) : Response{
		...
		$user = null;
		$scheme = null;
		if (isset($params['REDIRECT_HTTP_AUTHORIZATION']) && strlen($params['REDIRECT_HTTP_AUTHORIZATION'])){
			list($scheme, $userAndPassword) = explode(' ', trim($params['REDIRECT_HTTP_AUTHORIZATION']), 2);
			list($user, $password) = explode(':', base64_decode($userAndPassword), 2);
		}
		if ($scheme != 'Basic' || !$user || !$password || $user != $this->user || !password_verify($password, $this->password)){
			$response = $response->withStatus(401);
			$response = $response->withHeader('WWW-Authenticate', 'Basic realm="BilancioCivico"');
			return $response;
		}
