...
use \ReddeRationem\BilancioCivico\Queries\SelectSubdivisions;
use \Twig_Environment as Environment;
class GetSubdivisionsAsHtml{
	private $environment = null;
	private $selectSubdivision = null;
	public function __construct(Environment $environment, SelectSubdivisions $selectSubdivisions){
		$this->environment = $environment;
		$this->selectSubdivisions = $selectSubdivisions;
	}
	public function __invoke(Request $request, Response $response, callable $next) : Response{
		...
		if (isset($queryParams['match'])){
			...
			$arguments['subdivisions'] = $this->selectSubdivisions->submit($arguments);
			$arguments['match'] = $queryParams['match'];
			$response->getBody()->write($this->environment->render('subdivisionindex.html', $arguments));
			return $response;
		}
		$arguments = [
			'cycleCode' => (int)$request->getAttribute('cycleCode'),
		];
		$response->getBody()->write($this->environment->render('subdivisionsummary.html', $arguments));
		return $response;
	}
}