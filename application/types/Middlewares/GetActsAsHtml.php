...
use \ReddeRationem\BilancioCivico\Queries\SelectActs;
use \Twig_Environment as Environment;
class GetActsAsHtml{
	private $environment = null;
	private $selectAct = null;
	public function __construct(Environment $environment, SelectActs $selectActs){
		$this->environment = $environment;
		$this->selectActs = $selectActs;
	}
	public function __invoke(Request $request, Response $response, callable $next) : Response{
		...
		if (isset($queryParams['match'])){
			...
			$arguments['acts'] = $this->selectActs->submit($arguments);
			$arguments['match'] = $queryParams['match'];
			$response->getBody()->write($this->environment->render('actindex.html', $arguments));
			return $response;
		}
		$arguments = [
			'cycleCode' => (int)$request->getAttribute('cycleCode'),
		];
		$response->getBody()->write($this->environment->render('actsummary.html', $arguments));
		return $response;
	}
}