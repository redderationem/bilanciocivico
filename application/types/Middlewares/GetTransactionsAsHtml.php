...
use \ReddeRationem\BilancioCivico\Queries\SelectTransactions;
use \Twig_Environment as Environment;
class GetTransactionsAsHtml{
	private $environment = null;
	private $selectTransactions = null;
	public function __construct(Environment $environment, SelectTransactions $selectTransactions){
		$this->environment = $environment;
		$this->selectTransactions = $selectTransactions;
	}
	public function __invoke(Request $request, Response $response, callable $next) : Response{
		...
		if (isset($queryParams['match'])){
			...
			$arguments['transactions'] = $this->selectTransactions->submit($arguments);
			$arguments['match'] = $queryParams['match'];
			$response->getBody()->write($this->environment->render('transactionindex.html', $arguments));
			return $response;
		}
		$arguments = [
			'cycleCode' => (int)$request->getAttribute('cycleCode'),
		];
		$response->getBody()->write($this->environment->render('transactionsummary.html', $arguments));
		return $response;
	}
}