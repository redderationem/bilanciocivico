...
use \ReddeRationem\BilancioCivico\Queries\SelectTransaction;
use \Twig_Environment as Environment;
class GetTransactionAsHtml{
	private $environment = null;
	private $selectTransaction = null;
	public function __construct(Environment $environment, SelectTransaction $selectTransaction){
	...
	public function __invoke(Request $request, Response $response, callable $next) : Response{
		...
		$arguments['transaction'] = $this->selectTransaction->submit($arguments);
		$response->getBody()->write($this->environment->render('transaction.html', $arguments));
		return $response;
	}
}