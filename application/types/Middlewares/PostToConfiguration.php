...
use \ReddeRationem\BilancioCivico\Commands\Configure;
class PostToConfiguration{
	private $clear = null;
	private $configure = null;
	public function __construct(Clear $clear, Configure $configure){
		$this->clear = $clear;
	...
	public function __invoke(Request $request, Response $response, callable $next) : Response{
		...
		$arguments = [
			...
			'siteManager' => $parsedBody['siteManager'],
			'siteTitle' => $parsedBody['siteTitle'],
			'user' => $parsedBody['user']
		];
		$this->configure->submit($arguments);
		$this->clear->submit();
