...
use \ReddeRationem\BilancioCivico\Queries\SelectAct;
use \Twig_Environment as Environment;
class GetActAsHtml{
	private $environment = null;
	private $selectAct = null;
	public function __construct(Environment $environment, SelectAct $selectAct){
	...
	public function __invoke(Request $request, Response $response, callable $next) : Response{
		...
		$arguments['act'] = $this->selectAct->submit($arguments);
		$response->getBody()->write($this->environment->render('act.html', $arguments));
		return $response;
	}
}