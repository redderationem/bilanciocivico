...
class GetStylesheetAsCss{
	private $parser = null;
	private $directories = null;
	private $variables = null;
	public function __construct(array $directories, Parser $parser, array $variables){
	...
	public function __invoke(Request $request, Response $response, callable $next) : Response{
		...
		foreach ($this->directories as $v){
			$v = $v . '/' . $attributes['name'] . '.less';
			if (file_exists($v)){
				$response = $response->withHeader('Content-Type', 'text/css');
				// Doesn't work inside phar because it internally uses realpath
				// $this->parser->parseFile($v . '/' . $name);
				$this->parser->parse(file_get_contents($v));
				$this->parser->ModifyVars($this->variables);
				$response->getBody()->write($this->parser->getCss());
				return $response;
			}
		}
		throw new Exception();
