...
use \ReddeRationem\BilancioCivico\Queries\SelectSubjects;
use \Twig_Environment as Environment;
class GetSubjectsAsHtml{
	private $environment = null;
	private $selectSubjects = null;
	public function __construct(Environment $environment, SelectSubjects $selectSubjects){
	...
	public function __invoke(Request $request, Response $response, callable $next) : Response{
		...
		if (isset($queryParams['match'])){
			...
			$arguments['subjects'] = $this->selectSubjects->submit($arguments);
			$arguments['match'] = $queryParams['match'];
			$response->getBody()->write($this->environment->render('subjectindex.html', $arguments));
			return $response;
		}
		$arguments = [
			'cycleCode' => (int)$request->getAttribute('cycleCode'),
		];
		$response->getBody()->write($this->environment->render('subjectsummary.html', $arguments));
		return $response;
	}
}