...
class SelectSubjects{
	...
	public function submit(array $arguments = []){
		if (!isset($arguments['by']) || !in_array($arguments['by'], ['amount', 'code', 'count', 'name'])){
		...
		$sql = <<<SQL
		SELECT COUNT("denormalizedSubjects"."id") AS "count", ABS(SUM("denormalizedSubjects"."amount")) AS "amount"
		FROM "denormalizedSubjects"
		WHERE 
			(
				? IS NULL 
				OR
				"denormalizedSubjects"."id" IN (
					SELECT "indexedDenormalizedSubjects"."rowid"
					FROM "indexedDenormalizedSubjects"
					WHERE "indexedDenormalizedSubjects" MATCH ?
				)
			)
			AND 
			"denormalizedSubjects"."cycleCode" = ?
			AND 
			"amount" $operator 0
SQL;
		...
		$sql = <<<SQL
		SELECT 
			ABS("denormalizedSubjects"."amount") AS "amount",
			"denormalizedSubjects"."code",
			"denormalizedSubjects"."count",
			"denormalizedSubjects"."name",
			"denormalizedSubjects"."id"
		FROM "denormalizedSubjects"
		WHERE
			(
				? IS NULL 
